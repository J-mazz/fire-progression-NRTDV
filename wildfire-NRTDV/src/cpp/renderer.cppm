...
static bool context_lost_callback(int eventType, const void* reserved, void* userData) {
    (void)eventType;
    (void)reserved;
    (void)userData;
    g_context_lost = true;
...
static bool context_restored_callback(int eventType, const void* reserved, void* userData) {
    (void)eventType;
    (void)reserved;
    (void)userData;
...
export int initialize_webgl_context() {
...
export int update_geometry_from_vertices(const std::vector<float>& vertex_data) {
    if (g_context_lost || vertex_data.empty()) {
        return 0;
    }
    ...
    g_geometry.vertex_count = static_cast<uint32_t>(vertex_data.size() / 3);
    return g_geometry.vertex_count > 0 ? 1 : 0;
}

export void render_frame() {
    if (!g_context_ready || g_geometry.vertex_count == 0u) {
        return;
    }
    main_loop_callback(nullptr);
}

export bool is_context_lost() {
    return g_context_lost;
}

export bool is_context_ready() {
