...
def polygonize_masks(masks, transform, frame_time, scene_time, model_name):
    features = []
...
def main():
    ...
    for observation in firms:
        ...
        geojson = polygonize_masks(selected_masks, transform, frame_time, scene["observedAt"], args.model)
    ...
    print(f"Published {len(sentinel_observations)} Sentinel scenes and {len(sam_observations)} SAM-2 hotspot masks.")
